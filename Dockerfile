...
# .npmrc points @northgate at 4873 already; only the scope registry is rewritten here.
RUN npm config set @northgate:registry "${NPM_REGISTRY}" && npm ci --no-audit --no-fund
COPY . .
...
COPY --from=build /workspace/dist/northgate-business /usr/share/nginx/html
